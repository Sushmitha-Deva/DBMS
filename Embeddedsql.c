...
void main() {
   ...
   char *password = "sudeeshna12"; /* set me first - pwd : iiitdm123 */ 
...
char name[20],Minit,lname[20],address[20],sex;
...
scanf("%d",&dno);
snprintf(statement, 512, "INSERT INTO employee VALUES ('%s','B','ii',%d,'%d-%d-%d','453 ,Houseton','M',%d,%d,%d)", name,ssn,yr,mnth,date,sal,sssn,dno);
...
scanf("%d",&salary);
snprintf(statement, 1024, "UPDATE employee SET salary=%d  WHERE ssn=%d",salary,ssn);
mysql_query(conn, statement);
 */
